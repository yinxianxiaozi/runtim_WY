...
static void methodizeClass(Class cls, Class previously)
{
    ...
    if (list) {
        ...
        //这个方法的意思是附着方法列表到原有列表上
        //这里的做法是将原始类的方法列表附着到rwe的方法列表上
        //
        /*
...
findMethodInSortedMethodList(SEL key, const method_list_t *list)
{
    ...
    for (count = list->count; count != 0; count >>= 1) {
        /*
         ...
         每一次循环需要同构起始位置偏移count/2的位置得到中间位置
         */
...
static void resolveInstanceMethod(id inst, SEL sel, Class cls)
{
    ...
    //再查询一次得到imp
    //此时虽然会查询一边，但是因为lookUpImpOrNil函数中的behavior固定为1100，所以不会再次进行动态方法解析了
    IMP imp = lookUpImpOrNil(inst, sel, cls);
    //只有yes才会进入
    if (resolved  &&  PrintResolving) {
        //进行打印
...
IMP lookUpImpOrForward(id inst, SEL sel, Class cls, int behavior)
{
    /*
     ...
     behavior也是其中的某个值
     因此behavior与这几个数相与，只有相等，才会不会0，如果不相等肯定会为0
     */
    ...
    /*
        ...
        behavior & LOOKUP_INITIALIZE说明此处是进行查找初始化方法，并且该方法不是初始化方法
        初始化，执行initialize函数
        这里可以看出只有在查找方法列表时才会调用initialize函数
     
        所以条件为：cache中没找到进入到方法列表中查找方法，且该类还没有被初始化
     */
