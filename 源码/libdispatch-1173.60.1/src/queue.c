...
libdispatch_init(void)
{
	...
	_dispatch_queue_set_current(&_dispatch_main_q);//设置为当前队列
	_dispatch_queue_set_bound_thread(&_dispatch_main_q);//使用主线程
	...
	_os_object_init();//这里是开启了object_init的执行，会进一步进入到objc_init方法
	_voucher_init();
